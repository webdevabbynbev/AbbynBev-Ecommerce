...
export type SaleRecord = {
  ...
  products?: Array<{
    id: number;
    name: string;
    pivot?: { sale_price: number; stock: number };
  }>;
};
...
const FormSale: React.FC<Props> = ({ data, handleClose }) => {
  ...
  const loadProducts = React.useCallback(
    async (q?: string) => {
      try {
        ...
        if (keyword) qs.set("name", keyword);
        qs.set("page", "1");
  ...
  const syncSelectedProducts = React.useCallback(
    (idsRaw: any[]) => {
      ...
      const currentList = (form.getFieldValue("products") || []) as SaleProductInput[];
      const map = new Map<number, SaleProductInput>();
      for (const row of currentList) map.set(Number(row.product_id), row);

      const nextList: SaleProductInput[] = ids.map((id) => {
  ...
  React.useEffect(() => {
    ...
    const initialProducts =
      data.products?.map((p) => ({
        product_id: p.id,
        sale_price: p.pivot?.sale_price ?? 1,
    ...
    Modal.error({
      ...
      content: (
        <div>
          ...
          <div>
            Solusi: nonaktifkan/ubah jadwal Discount, atau ubah periode Sale.
          </div>
        </div>
      ),
    });
